...
def plot_mental_health_conditions_by_gender_raw(
    ...
    for i, bar_group in enumerate(ax.containers):
        ...
        for bar in bar_group:
            bar.set_color(
                gender_colors.get(gender, "gray")
            )  # Default to gray if not specified
...
def calculate_condition_statistics_by_gender_optimized(
    db_path, condition_mapping, gender_mapping
):
    """
    Calculates statistics on selected mental health conditions by gender, optimized for speed.
    ...
    Returns:
        pd.DataFrame: A DataFrame containing the calculated statistics for each gender group.
    ...
    try:
        ...
        SELECT 
            ...
            a1.AnswerText AS gender,
            a3.AnswerText AS age
        FROM Answer a1
        JOIN Question q1 ON a1.QuestionID = q1.questionid
        JOIN Answer a2 ON a1.SurveyID = a2.SurveyID
        JOIN Question q2 ON a2.QuestionID = q2.questionid
        LEFT JOIN Answer a3 ON a1.SurveyID = a3.SurveyID
        LEFT JOIN Question q3 ON a3.QuestionID = q3.questionid
        WHERE q1.questiontext LIKE 'What is your gender%'
        AND q2.questiontext LIKE 'If yes, what condition(s) have you been diagnosed with%'
        AND q3.questiontext = 'What is your age?';
        """
        ...
        pivoted = grouped.pivot_table(
            index="gender", columns="Category", values="percentage", fill_value=0
        ).reset_index()
        age_stats = (
            df.groupby("gender")
            .agg(median_age=("age", "median"), average_age=("age", "mean"))
            .reset_index()
        )
        total_count = df.groupby("gender").size().reset_index(name="Total Count")

        result = pd.merge(pivoted, age_stats, on="gender")
        result = pd.merge(result, total_count, on="gender")
        result = result.rename(columns={"gender": "Gender"})
