...
def plot_numeric_histograms(df: pd.DataFrame, numeric_cols: List[str]) -> None:
    """
    Plots histograms with KDE for a list of numeric columns in a DataFrame.
...
def plot_correlation_heatmap(
    ...
    """
    Plots a correlation heatmap of specified numerical features and the target column.

    Args:
        df (pd.DataFrame): The dataset containing the columns.
        numeric_cols (List[str]): List of numerical feature names to include in the heatmap.
        target_col (str): The name of the target column to include in the correlation matrix.

    Returns:
        None
    """
    df_numeric = df[numeric_cols + [target_col]]
...
def plot_violin_numerical_by_target(
    ...
    for i, col in enumerate(numeric_cols):
        ...
        axes[i].set_title(f"{col} by {target_col}", fontsize=14)
...
def test_income_difference(df: pd.DataFrame) -> None:
    ...
    using Welch's t-test (with Shapiro-Wilk test for normality).
    ...
    if (
        ...
        print(
            "\nNormality not met and sample size is small. Consider using a non-parametric test."
        )
...
def plot_confidence_intervals() -> None:
    """
    Visualizes the 95% confidence intervals for differences in proportions (e.g.,
    travel vs. no travel, graduates vs. non-graduates) and income differences using bar plots.
    ...
    prop_diffs = [0.251, 0.041]
    prop_errors = [0.065, 0.073]
    prop_cis = [(0.305, 0.435), (-0.030, 0.114)]

    income_label = ["Insured vs Uninsured"]
    income_diff = [217620.78]
    income_errors = [258189.80 - 217620.78]
    ...
    axes[1].bar(x_income, income_diff, yerr=income_errors, capsize=8, color=[colors[2]])
    ...
    fig.legend(
        ...
        labels=["Travel Abroad Experience", "Graduation", "Income"],
        loc="upper center",
...
def load_and_clean_data(filepath: str) -> pd.DataFrame:
    """
    Loads a dataset from a CSV file, removes duplicate rows and unnecessary index columns.
...
def plot_model_comparison(metrics: Dict[str, List]) -> None:
    """
    Plot a bar chart comparing performance metrics across multiple classification models.
    ...
    Example:
        metrics = {
            ...
            "ROC AUC": [0.62, 0.59, 0.67, 0.74, 0.69],
        }
    ...
    sns.barplot(data=df_melted, x="Model", y="Score", hue="Metric", palette="mako_r")
...
def plot_f1_comparison(
    ...
    """
    Plots F1 Score comparison before and after hyperparameter tuning for given models.
