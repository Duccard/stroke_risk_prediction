...
def plot_roc_curve(
    ...
    """
    Plots the ROC curve for a given classification model.

    Parameters:
    - model: trained classifier with predict_proba method
    - X: feature set (DataFrame or array)
    - y: true labels
    - label_desc: description to show in legend (e.g., "Random Forest")
    - title: title for the plot
    - palette: seaborn color palette (default: PuRd)
    - auc_pos: index in palette for curve color
    """

    probs = model.predict_proba(X)
    class_index = list(model.classes_).index(1)
    y_probs = probs[:, class_index]

    fpr, tpr, _ = roc_curve(y, y_probs)
    auc_score = roc_auc_score(y, y_probs)

    palette_colors = sns.color_palette(palette, max(auc_pos + 1, 6))
    plt.figure(figsize=(8, 6))
    plt.plot(fpr, tpr, label=f"AUC = {auc_score:.2f}", color=palette_colors[auc_pos])
    plt.plot([0, 1], [0, 1], linestyle="--", color="gray")
    plt.xlabel("False Positive Rate")
    plt.ylabel("True Positive Rate")
    plt.title(f"{title} ({label_desc})")
    plt.legend(loc="lower right")
    plt.grid(True)
    plt.tight_layout()
    plt.show()
...
def plot_feature_importances(
    model, feature_names, palette="PuRd_r", title="Feature Importances"
):
    """
    Plots feature importances for a trained tree-based model (e.g., Random Forest).
    ...
    sns.barplot(
        x="Importance", y="Feature", hue="Feature", data=importance_df, palette=palette
    )
    plt.title(title)
